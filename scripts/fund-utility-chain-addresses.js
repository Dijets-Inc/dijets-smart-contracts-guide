...
const dijets_1 = require('dijets');
const avm_1 = require('dijets/dist/apis/avm');
const evm_1 = require('dijets/dist/apis/evm');
const utils_1 = require('dijets/dist/utils');
const sleep = (ms) => {
...
const dijets = new dijets_1.Avalanche(ip, port, protocol, networkID);
const mstimeout = 3000;
const xchain = dijets.XChain();
const cchain = dijets.CChain();
const bintools = dijets_1.BinTools.getInstance();
...
const locktime = new dijets_1.BN(0);
const threshold = 1;
...
const main = async () => {
  ...
  const balance = new dijets_1.BN(getBalanceResponse.balance);
  ...
  secpTransferOutput = new avm_1.SECPTransferOutput(
    balance.sub(amount.mul(new dijets_1.BN(10))).sub(fee),
    xAddresses,
    locktime,
    threshold
  );
